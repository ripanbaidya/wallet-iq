...
> = {
  ...
  RECURRING_TRANSACTION: { icon: "↻", color: "text-blue-700",   bg: "bg-blue-50"   },
  ...
  return (
    <div className="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors group">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[13px] text-gray-800 leading-snug">{notification.message}</p>
...
const NotificationBell: React.FC<Props> = ({ size = "md", align = "left" }) => {
  ...
  // ── Delete all ─────────────────────────────────────────────────────────────
  const { mutate: deleteAll, isPending: isDeletingAll } = useAppMutation<void, void>({
