...
export default function SavingsPage() {
  ...
  const filtered =
    statusFilter === "ALL"
      ? allGoals
      : allGoals.filter((g: any) => g.status === statusFilter);

  // Counts per status for tab badges
  const counts = {
    ALL: allGoals.length,
    IN_PROGRESS: allGoals.filter((g: any) => g.status === "IN_PROGRESS").length,
    ACHIEVED: allGoals.filter((g: any) => g.status === "ACHIEVED").length,
    FAILED: allGoals.filter((g: any) => g.status === "FAILED").length,
  };
