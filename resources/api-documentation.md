# RAG-Based Expense Tracker — Backend Architecture Guide
...
When a transaction is saved, you generate a text representation (e.g., *"Spent ₹450 on Food via Card on 2025-03-01. Note: lunch with team"*) and store its vector in the vector DB. `embeddingId` links the relational row to its vector document so you can delete/update embeddings when transactions change.
